...
- Hidden member upload page: submit.html (noindex, code 2010, checked in the browser AND in the Apps Script). Reached only from the footer logo on gallery.html; the footer logo on every other page goes home. Uploads go through the Light Writerz-only Apps Script (tools/apps-script/lwz-incoming.gs) into LWZ_incoming as STUDENT_name_title files. ENDPOINT in js/submit.js is empty until Chris deploys the script. Light Writerz and Creative Silva uploaders are deliberately SEPARATE deployments; never merge them.
